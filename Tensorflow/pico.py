...
BASE_DIR = Path("training_realworld_tensorflow_picologging")
...
def setup_console_logger(name: str = "tf_batch_logger_picologging"):
...
def train_picologging_batch_logging(x_train, y_train, x_test, y_test, epochs):
    model = build_model()
...
def main():
    ...
    for epochs in EPOCH_LIST:
        print(f"\n{'=' * 25} RUNNING PICOLOGGING EPOCHS = {epochs} {'=' * 25}")

        result = run_with_codecarbon(
            experiment_name=f"train_picologging_batch_logging_epochs{epochs}",
            emissions_file=f"picologging_batch_logging_epochs{epochs}_emissions.csv",
            train_fn=lambda e=epochs: train_picologging_batch_logging(
                x_train, y_train, x_test, y_test, e
            ),
            epochs=epochs,
            condition="picologging_batch_logging",
        )
    ...
    summary_path = BASE_DIR / "training_summary_picologging_only.csv"
